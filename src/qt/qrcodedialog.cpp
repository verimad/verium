...
QString QRCodeDialog::getURI()
{
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non VRM unit input in lnReqAmount, we generate the URI with VRM as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::formatMaxDecimals(BitcoinUnits::VRM, ui->lnReqAmount->value(), BitcoinUnits::maxdecimals(BitcoinUnits::VRM)));
            paramCount++;
        }
        else
        {
