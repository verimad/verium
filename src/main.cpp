...
CBigNum bnProofOfWorkLimit(~uint256(0) >> 8);  // standard scrypt-hard minimum difficulty (0.00000095)
...
const string strMessageMagic = "Verium Signed Message:\n";
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    if (pblock->hashPrevBlock != hashBestChain)
    {
        ...
        bnRequired.SetCompact(ComputeMinWork(GetLastBlockIndex(pcheckpoint)->nBits, deltaTime));
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Verium", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        block.nNonce   = !fTestNet ? 228934 : 228934;
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    if (inv.hash == pfrom->hashContinue)
                    {
                        ...
                        vector<CInv> vInv;
                        vInv.push_back(CInv(MSG_BLOCK, GetLastBlockIndex(pindexBest)->GetBlockHash()));
